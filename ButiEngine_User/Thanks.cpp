...
void ButiEngine::Thanks::OnUpdate()
{
	...
	if (shp_appleGenerateTimer->Update()) {
		...
		auto apple= GetManager().lock()->AddObjectFromCereal("ClearApple", ObjectFactory::Create<Transform>(Vector3(pos.x, pos.y,1.0f+ 0.01*appleCount), Vector3::Zero, Vector3(0, 0, 1)));
...
void ButiEngine::Thanks::Start()
{
	...
	shp_startTimer= ObjectFactory::Create<RelativeTimer>(44);
	shp_startTimer->Start();
}
