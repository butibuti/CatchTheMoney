...
namespace ButiEngine {
	...
	public:
		~MeshHelper();
		static void ReverseWinding(std::vector<Vertex::Vertex_UV_Normal_Color>& ref_vertices, std::vector<unsigned int>& ref_indices);
		...
		static void CreateCylinderCap(const std::vector<Color>& arg_colors,std::vector<Vertex::Vertex_UV_Normal_Color>& ref_vertices, std::vector<unsigned int>& ref_indices, Vector3 size, int tessellation, bool isTop);
		...
		static void CreatePlane(Vector2 size, Vector3 offset,float tilt,  float UVMax, const unsigned int arg_verticalSeparate, const unsigned int arg_horizontalSeparate, const std::vector<Color>& arg_colors, bool flat, BackupData<Vertex::Vertex_UV_Normal_Color>& outputMeshData);
		...
		static void CreateCirclePolygon(const float radius,const unsigned int	tessellation, BackupData<Vertex::Vertex_UV_Normal_Color>& outputMeshData);
		...
		static void CreateImmediateMeshForParticle(const unsigned int arg_particleCount, BackupData<Vertex::Vertex_UV_Normal_Color>& outputMeshData);
	private:
		MeshHelper();
	};
}
