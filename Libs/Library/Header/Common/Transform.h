...
namespace ButiEngine {
	...
	public:
		...
		inline void SetLocalPostionX(const float arg_value) {
			if (localMatrix) {
				localMatrix->_41 = arg_value;
			}
			localPosition.x = arg_value;
		}
		inline void SetLocalPostionY(const float arg_value) {
			if (localMatrix) {
				localMatrix->_42 = arg_value;
			}
			localPosition.y = arg_value;
		}
		inline void SetLocalPostionZ(const float arg_value) {
			if (localMatrix) {
		...
		inline const void SetWorldPostionX(const float arg_value) {
			auto currentPos = GetWorldPosition();
			currentPos.x = arg_value;
			SetWorldPosition(currentPos);
		}
		inline const void SetWorldPostionY(const float arg_value) {
			auto currentPos = GetWorldPosition();
			currentPos.y = arg_value;
			SetWorldPosition(currentPos);
		}
		inline const void SetWorldPostionZ(const float arg_value) {
			auto currentPos = GetWorldPosition();
